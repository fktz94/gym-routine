...
const EditExerciseModal = ({
  ...
  const { handleEditOneExercise, newRoutineState } = useNewRoutineContext();

  const selectedExercise = newRoutineState.data[dayIndex].find((el) => el.name === exerciseName);
  ...
  const [hasWeeksVariations, setHasWeeksVariations] = useState(
    selectedExercise.weightsAndRepetitions.length > 1
  );
  const [isCustomRepetitions, setIsCustomRepetitions] = useState(false);
  const styles = editExerciseModalStyles(theme, hasWeeksVariations);

  const [variations, setVariations] = useState<WeightsAndRepetitions[]>(
    selectedExercise.weightsAndRepetitions
  );
...
const editExerciseModalStyles = (theme: Theme, multipleRepetitions: boolean) =>
  StyleSheet.create({
    closeIconBtn: {
      position: "absolute",
      right: 0,
